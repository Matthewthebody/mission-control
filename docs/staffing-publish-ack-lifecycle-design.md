...
"Idempotent" applies to the **version / recipients / notifications** only — an unchanged-plan publish still runs the **canonical shift-publication loop** (a non-material draft edit must still be published; *"unchanged plan" ≠ "nothing to publish"*). The prior aggregate "staffing published" notification is **preserved** alongside the per-recipient version-aware outbox record (the per-recipient dispatch that supersedes the coarse aggregate is sub-slice 4). All writes — version, recipients, canonical publish mutations, app_event, notification outbox — share the one publish transaction, so a failure rolls everything back atomically.

## 7. Decline → canonical readiness (LOCKED)
A declined assignment must **not** continue satisfying accepted operational readiness just because its `work_shift` is still `published`. **Keep raw assignment truth separate from accepted readiness** — do not silently redefine `assigned_staff_count` as an accepted count. The coverage SQL (`getStaffingDashboardOverview`/`mapCoverageRow`) and `listSchedulingUrgentWatchCandidates` keep the **raw** `assigned_staff_count` + `lead_coverage_count` and add explicit **`accepted_staff_count` / `accepted_lead_coverage` / `declined_staff_count` / `declined_lead_name` / `lead_assigned` / `lead_ready` / `replacement_required`**. Readiness (`missing_lead` / `under_staffed` / candidate gap) derives from the **accepted** counts, while the raw counts + the declined employee name stay visible — the manager view shows "Lead: Sarah — Declined; replacement required", never "nobody assigned".
