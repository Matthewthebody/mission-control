...
A declined assignment must **not** continue satisfying operational readiness just because its `work_shift` is still `published`. **Keep raw assignment truth separate from readiness** — do not silently redefine `assigned_staff_count`. The coverage SQL (`getStaffingDashboardOverview`/`mapCoverageRow`) and `listSchedulingUrgentWatchCandidates` keep the **raw** `assigned_staff_count`, `published_staff_count`, `lead_assignment_count` (= `lead_coverage_count`), `lead_name` and add explicit **`readiness_coverage_count` / `readiness_lead_coverage_count` / `declined_staff_count` / `declined_lead_name` / `readiness_lead_name` / `lead_assigned` / `lead_readiness_met` / `replacement_required`**. Readiness (`missing_lead` / `under_staffed` / candidate gap) derives from the **readiness** counts. Terminology matters: readiness coverage = assigned **AND not currently declined** — a still-**pending** (awaiting-acknowledgment) employee counts toward coverage but is deliberately **not** called "accepted" (acknowledgment hasn't occurred). The raw counts + the declined employee name stay visible — the manager view supports "Lead: Sarah — Awaiting acknowledgment", "Lead: Sarah — Acknowledged", and "Lead: Sarah — Declined; replacement required", never "nobody assigned".
