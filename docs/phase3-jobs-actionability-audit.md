...
- **Slice 3 — Job-count coherence.** Reconcile the event-driven vs state-driven "behind" definitions into one canonical number, and align Project Tracking's backend summary with the front-end's filtered group counts. *May touch a read model; audit-first.*
  - **Card-vs-drilldown coherence — RESOLVED (Slice 1 follow-up).** The job-count cards' `#jobs?<filter>` drilldown originally landed on `SharedJobsPage`, which fetched up to 200 jobs once and filtered client-side, so the blocked filter showed only 4 of the 7 blocked jobs (the rest sorted past row 200). Closed by adding optional **server-side `production_status` / `readiness_status` filters** to `GET /api/jobs` (`listJobs`, text-compared so unrecognized values match nothing) and having `SharedJobsPage` forward the deep-linked **global-page** status filter to the fetch (guarding the overloaded calendar-readiness values, which stay client-side). A deep-linked filtered view now returns the full matching set, so a card's count equals exactly what its drilldown shows. Proven by `jobStatusCounts.test.ts` (count == `#jobs?production_status=blocked` length) and `sharedJobPages.test.tsx`.
  - **Still open for Slice 3:** the deeper event-driven (`urgent_watch_item`) vs state-driven (workflow health) "behind" reconciliation, and Project Tracking summary-vs-group count alignment.
