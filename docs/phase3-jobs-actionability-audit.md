...
- **Slice 3 — Job-count coherence.** Reconcile the event-driven vs state-driven "behind" definitions into one canonical number, and align Project Tracking's backend summary with the front-end's filtered group counts. *May touch a read model; audit-first.* **Carry-in from Slice 1:** the canonical job-count cards now show accurate, uncapped counts (e.g. `production_status=blocked` = 7), but their `#jobs?<filter>` drilldown lands on `SharedJobsPage`, which fetches up to 200 jobs once and filters client-side — so for the 297-job demo tenant the blocked filter shows only the 4 blocked jobs that fall within the first 200 by sort order. Closing exact count==destination coherence means giving the Jobs index a server-side status filter (or a higher/eliminated cap) so a deep-linked filtered view returns the full matching set. Deferred here because it reworks the page's load-200/client-filter data model, which is out of Slice 1's frontend-led scope.
