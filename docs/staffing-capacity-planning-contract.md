...
- **Window clipping (every total):** before any minute is summed, each assignment interval is **intersected with the requested operating window** (`clipInterval`). An assignment that straddles the boundary contributes only its in-window portion; one entirely outside contributes **zero**; one spanning the whole window contributes at most the window's own minutes. The canonical `starts_at`/`ends_at` are **never** mutated/truncated/cancelled — only the *counted* portion is clipped. This makes totals truthful even when corrupt/legacy multi-year `work_shift` rows exist (they can never pollute a weekly total with millions of minutes).
- **Data quality (per assignment):** `timing_quality` ∈ `valid | incomplete | suspicious`; `source_duration_minutes` (full canonical length), `clipped_duration_minutes` (what counts), `timing_warning_reason`. A single shift longer than `config.CAPACITY_SUSPICIOUS_SHIFT_MINUTES` (default `DEFAULT_SUSPICIOUS_SHIFT_MINUTES` = 24h, centralized — no scattered magic numbers) is **suspicious**: still clipped + counted, but flagged (employee `suspicious_timing_count`, summary `suspicious_timing_count`) and surfaced in the UI — never silently dropped or silently inflating a total.
