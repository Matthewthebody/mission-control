...
describe("StudiosWorkspace", () => {
  ...
  it("renders Photography as a workflow-first command center with contextual shoot links", async () => {
    ...
    expect(await screen.findByRole("heading", { level: 2, name: "Photography" })).toBeInTheDocument();
    expect(screen.getByText("Today's shoots, travel details, job prep, closeout, and senior review in one field-ready view.")).toBeInTheDocument();
    expect(screen.getByText("Open First")).toBeInTheDocument();
    expect(screen.getByText("Start with the field work that matters today")).toBeInTheDocument();
    expect(screen.getAllByText("Today's Shoots").length).toBeGreaterThan(0);
    expect(screen.getAllByText("Team Schedule").length).toBeGreaterThan(0);
    expect(screen.getByText("Travel")).toBeInTheDocument();
    expect(screen.getAllByText("Job Prep").length).toBeGreaterThan(0);
    ...
    expect(screen.getAllByText("North Metro Stadium").length).toBeGreaterThan(0);
    expect(screen.getAllByText("Carisa Lead").length).toBeGreaterThan(0);
    ...
    expect(screen.getByRole("heading", { level: 3, name: "This Week's Photography Schedule" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Today" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "This Week" })).toHaveClass("is-active");
    expect(screen.getByRole("button", { name: "Staffing" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Prep Gaps" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Open Team Schedule" })).toBeInTheDocument();
    expect(await screen.findByRole("heading", { level: 3, name: "Photography Work Queue" })).toBeInTheDocument();
    expect(screen.getByLabelText("Photography Work Queue summary")).toBeInTheDocument();
    ...
    fireEvent.click(screen.getByRole("button", { name: "Open Team Schedule" }));
    await waitFor(() => {
  ...
  it("renders Job Prep as the consolidated Pre-Service and Readiness packet", async () => {
    ...
    expect(await screen.findByRole("heading", { level: 2, name: "Job Prep" })).toBeInTheDocument();
    ...
    expect(screen.getByRole("heading", { level: 4, name: "Choose Job" })).toBeInTheDocument();
    expect(screen.getAllByRole("heading", { name: "Selected Job Summary" }).length).toBeGreaterThan(0);
    ...
    expect(screen.getByRole("heading", { level: 4, name: "Travel & Location" })).toBeInTheDocument();
    expect(screen.getAllByText("North Metro Stadium").length).toBeGreaterThan(0);
    ...
    expect(screen.getByRole("heading", { level: 4, name: "Location Details" })).toBeInTheDocument();
    ...
    expect(screen.getByRole("link", { name: "Location Details" })).toHaveAttribute("href", "#directory/locations?location=loc-1");
    expect(screen.getByRole("heading", { level: 4, name: "Past Shoot References" })).toBeInTheDocument();
    ...
    await waitFor(() => {
      expect(window.location.hash).toBe("#studios/travel?job=job-prep");
    });

    expect(screen.queryByText("Reference Packet")).not.toBeInTheDocument();
    expect(screen.queryByText("No customer survey notes are in this seeded packet yet.")).not.toBeInTheDocument();
  ...
  it("keeps the old Readiness route as a thin compatibility path into Job Prep", async () => {
    ...
    expect(await screen.findByRole("heading", { level: 2, name: "Job Prep" })).toBeInTheDocument();
    expect(screen.getByText("Readiness is part of Job Prep. Use this packet for readiness, notes, resources, and crew context.")).toBeInTheDocument();
    expect(await screen.findByRole("heading", { level: 3, name: "North Metro Stadium Media Day" })).toBeInTheDocument();
    expect(screen.getByRole("heading", { level: 4, name: "Prep Readiness" })).toBeInTheDocument();
    expect(screen.queryByRole("button", { name: "Compact List" })).not.toBeInTheDocument();
  });
  ...
  it("renders a Photography-specific Today's Shoots route without generic create-task actions", async () => {
    ...
    expect(await screen.findByRole("heading", { level: 2, name: "Today's Shoots" })).toBeInTheDocument();
    expect(screen.getByRole("heading", { level: 3, name: "Today's Shoots" })).toBeInTheDocument();
  ...
  it("renders Travel as a field-ready overview with maps and without dead pilot controls", async () => {
    ...
    expect(await screen.findByRole("heading", { level: 2, name: "Travel" })).toBeInTheDocument();
    ...
    expect(screen.getByRole("button", { name: "Open Job Detail" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Open Project Tracking" })).toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: "Job Prep" }));
    await waitFor(() => {
      expect(window.location.hash).toBe("#studios/pre-service?job=job-travel");
    });
    ...
    expect(screen.queryByRole("button", { name: "Open Full Workspace" })).not.toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Back to Photography" })).toBeInTheDocument();
    expect(screen.queryByRole("button", { name: "Team Schedule" })).not.toBeInTheDocument();
    expect(screen.queryByRole("button", { name: "Today's Shoots" })).not.toBeInTheDocument();
  });
  ...
  it("routes studios shoots to the Photography Today surface", () => {
    ...
    expect(getRouteById(routeId)?.label).toBe("Today's Shoots");
  });
  ...
  it("keeps staffing and attendance owned by Leadership navigation instead of Photography", () => {
    ...
    expect(getRouteById("studios-pre-service")?.label).toBe("Job Prep");
  ...
  it("opens Photography calendar as a readable read-only 30-day view with a week toggle", async () => {
    ...
    expect(screen.getByText("Location: Lakeview High School")).toBeInTheDocument();
  ...
  it("keeps focused subpages above the repeated homepage launch stack and labels workload for senior photographers", async () => {
    ...
    expect(await screen.findByRole("heading", { level: 2, name: "Senior Review" })).toBeInTheDocument();
    ...
    expect(screen.getAllByRole("button", { name: "Today's Shoots" }).length).toBeGreaterThan(0);
    expect(screen.getAllByRole("button", { name: "Job Prep" }).length).toBeGreaterThan(0);
  });
});
